...
var (
	...
	ErrDuplicate = errors.New("data mahasiswa sudah ada (NIM ganda)")
)
...
// Daftar kolom yang diizinkan untuk ORDER BY demi mencegah SQL Injection di pengurutan
...
func (r *studentPostgresRepository) FindAll(ctx context.Context, q model.ListQuery) ([]model.Student, int, error) {
	...
	// 1. Hitung total baris untuk metadata paginasi
	var total int
	...
	defer rows.Close() // Wajib agar koneksi kembali ke pool
...
func (r *studentPostgresRepository) FindByNIM(ctx context.Context, nim string) (model.Student, error) {
	var s model.Student
	err := r.pool.QueryRow(ctx,
		"SELECT id, nim, name, grade, is_active, created_at FROM students WHERE nim = $1", nim,
	).Scan(&s.ID, &s.NIM, &s.Name, &s.Grade, &s.IsActive, &s.CreatedAt)
	...
	if err != nil {
		...
		return model.Student{}, fmt.Errorf("mengambil mahasiswa berdasarkan nim: %w", err)
	}
	return s, nil
}
...
// isUniqueViolation memeriksa kode error PostgreSQL 23505 (unique_violation)
